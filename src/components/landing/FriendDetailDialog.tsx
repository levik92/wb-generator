...
export const FriendDetailDialog = ({ friend, onClose }: FriendDetailDialogProps) => {
  return (
    <Dialog open={!!friend} onOpenChange={() => onClose()}>
      <DialogContent className="sm:max-w-2xl bg-[#111111] border-white/[0.08] text-white p-0 overflow-hidden max-h-[90vh] flex flex-col [&>button]:hidden">
        <DialogTitle className="sr-only">{friend?.name}</DialogTitle>
        
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-50 w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors"
        >
        ...
        <div className="overflow-y-auto flex-1 overscroll-contain">
          ...
          <div className="relative px-6 pt-8 pb-6 sm:px-8 sm:pt-10 sm:pb-8">
            ...
            <div className="relative flex items-start gap-5">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-white/[0.06] border border-white/[0.08] flex items-center justify-center overflow-hidden shrink-0">
                <img
                  src={friend?.logo_url}
                  alt={friend?.name}
