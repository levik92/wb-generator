...
const FriendBody = ({ friend, showClose, onClose }: { friend: ServiceFriend | null; showClose?: boolean; onClose: () => void }) => {
    ...
    return (
      <Drawer open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
        <DrawerContent className="bg-[#111111] border-white/[0.08] text-white">
          ...
          <DrawerDescription className="sr-only">Информация о партнёре</DrawerDescription>
          <div className="flex flex-col max-h-[80vh]">
            <FriendBody friend={friend} onClose={onClose} />
          </div>
        </DrawerContent>
      </Drawer>
    );
