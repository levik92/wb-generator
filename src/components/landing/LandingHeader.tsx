...
const productItems = [{
  ...
  icon: Image,
  description: "ИИ-дизайн за 3 минуты"
}, {
  label: "SEO-описания",
  href: "/seo-opisaniya",
  icon: FileText,
  description: "Продающие тексты с ключами"
}, {
  label: "Генератор ШК",
  href: "/generator-shk",
  icon: Barcode,
  description: "Штрихкоды бесплатно"
}, {
          ...
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            <nav className="hidden lg:flex items-center gap-6 order-2">
              ...
              <div className="relative" onMouseEnter={() => handleDropdownEnter("product")} onMouseLeave={handleDropdownLeave}>
                ...
                <AnimatePresence>
                  {openDropdown === "product" && <motion.div initial={{
                  opacity: 0,
                  y: 10
                }} animate={{
                  opacity: 1,
                  ...
                  duration: 0.2
                      ...
                      <div className="bg-[hsl(240,10%,10%)] border border-white/10 rounded-xl p-2 shadow-xl">
                        {productItems.map(item => <Link key={item.href} to={item.href} className="flex items-start gap-3 p-3 rounded-lg hover:bg-white/5 transition-colors group">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                ...
                                {item.isComingSoon && <span className="px-1.5 py-0.5 text-[10px] bg-amber-500/20 text-amber-400 rounded">Скоро</span>}
                              </div>
              ...
              <div className="relative" onMouseEnter={() => handleDropdownEnter("resources")} onMouseLeave={handleDropdownLeave}>
                ...
                <AnimatePresence>
                  {openDropdown === "resources" && <motion.div initial={{
                  opacity: 0,
                  y: 10
                }} animate={{
                  opacity: 1,
                  ...
                  duration: 0.2
                      ...
                      <div className="bg-[hsl(240,10%,10%)] border border-white/10 rounded-xl p-2 shadow-xl">
                        {resourceItems.map(item => item.external ? <Link key={item.href} to={item.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/5 transition-colors group">
                              <item.icon className="w-5 h-5 text-white/50 group-hover:text-white transition-colors" />
                              <span className="text-sm text-white/70 group-hover:text-white transition-colors">
                                {item.label}
                              </span>
                            </Link> : <Link key={item.href} to={item.href} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white/5 transition-colors group">
                              <item.icon className="w-5 h-5 text-white/50 group-hover:text-white transition-colors" />
                              <span className="text-sm text-white/70 group-hover:text-white transition-colors">
                                {item.label}
                              </span>
              ...
              <div className="flex flex-col h-full">
                ...
                <nav className="flex-1 p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {productItems.map(item => <Link key={item.href} to={item.href} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-white/80 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                          <item.icon className="w-5 h-5 text-white/50" />
                          <span>{item.label}</span>
                          {item.isComingSoon && <span className="ml-auto px-1.5 py-0.5 text-[10px] bg-amber-500/20 text-amber-400 rounded">Скоро</span>}
                        </Link>)}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {resourceItems.map(item => <Link key={item.href} to={item.href} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-white/80 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                          <item.icon className="w-5 h-5 text-white/50" />
                          <span>{item.label}</span>
                        </Link>)}
                    </div>
                ...
                <div className="p-4 border-t border-white/10 space-y-3">
                  <Link to="/auth?tab=signin" onClick={() => setIsMobileMenuOpen(false)}>
                    <Button variant="outline" className="w-full bg-white/5 border-white/10 text-white hover:bg-white/10 pb-0 mb-[10px]">
                      Войти
                    </Button>
                  </Link>
