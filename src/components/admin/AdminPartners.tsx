...
export const AdminPartners = () => {
  ...
  const loadTotalPaidOut = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("partner_withdrawals").select("amount").eq("status", "completed");
  ...
  const loadPartners = async () => {
    try {
      ...
      const {
        data: partnersData,
        error
      } = await supabase.from("partner_profiles").select("*").eq("status", "active").order("total_earned", {
        ascending: false
      });
      if (error) throw error;
      ...
      } = await supabase.from("partner_withdrawals").select("partner_id, amount, status").in("status", ["pending", "processing"]);
      ...
      if (userIds.length > 0) {
        ...
        if (profilesData) {
          profilesMap = Object.fromEntries(profilesData.map(p => [p.id, {
            email: p.email
          }]));
        }
      }
  ...
  const loadPartnerDetails = async (partner: PartnerData) => {
    ...
    try {
      ...
      const {
        data: withdrawalsData
      } = await supabase.from("partner_withdrawals").select("*").eq("partner_id", partner.id).order("requested_at", {
        ascending: false
      });
      ...
      const {
        data: bankData
      } = await supabase.from("partner_bank_details").select("*").eq("partner_id", partner.id).maybeSingle();
