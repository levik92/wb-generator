...
export default function WBLabelMakerAlt() {
  // ---- state ----
  const [code, setCode] = useState("ABC-abc-1234"); // штрих-код
  const [orderNum, setOrderNum] = useState(""); // Порядковый номер (title)
  const [freeText, setFreeText] = useState(""); // Свободное поле (sku)
  const [format, setFormat] = useState("a4"); // "a4" | "58x40"
  const [barWidth, setBarWidth] = useState(3); // 1..5
  ...
  const render = useCallback(async () => {
    ...
    // measure title (Порядковый номер)
    ...
    if (orderNum.trim()) {
      ...
      const words = orderNum.trim().split(/\s+/);
      let line = "";
      for (let w of words) {
    ...
    // free field line height
    const freeH = freeText.trim() ? Math.round(fmt.skuPx * 1.2) : 0;

    // barcode params
    const barHeight = Math.max(60, Math.round(usableH * (Number(barHPct) / 100)));
    const barLineW = Number(barWidth);

    // barcode canvas (full inner width for easy centering)
    ...
    // vertical centering
    const totalH = titleH + freeH + barHeight;
    let y = MT + Math.max(0, Math.floor((usableH - totalH) / 2));

    // draw order number (title)
    ...
    if (freeText.trim()) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(freeText.trim(), ML + innerW / 2, y + fmt.skuPx * 1.2);
      y += freeH;
    }
    ...
    // keep full-res for download
    (p as any).__fullImage = off;
  }, [PRESETS, format, barHPct, barWidth, code, orderNum, freeText, mm2px]);
  useEffect(() => {
    let raf: number;
    const onResize = () => {
  ...
  return <div id="wb-gen">
      {/* inline styles to match your Tilda block */}
      <style>{`
    ...
    .wb-prev-stage{min-height:420px}
      ...
      <div className="wb-grid">
        ...
        <div className="wb-card wb-form bg-background">
          ...
          <label className="wb-label">Порядковый номер</label>
          <input className="wb-input" placeholder="Введите порядковый номер" value={orderNum} onChange={e => setOrderNum(e.target.value)} />

          <label className="wb-label">Свободное поле</label>
          <input className="wb-input" placeholder="Введите произвольный текст" value={freeText} onChange={e => setFreeText(e.target.value)} />
          ...
          <div className="wb-row">
            <div className="wb-range">
              <div className="wb-range-top">
                <span>Толщина штрихов</span><span>{barWidth}</span>
              </div>
            ...
            <div className="wb-range">
              <div className="wb-range-top">
                <span>Высота штрих-кода</span><span>{barHPct}%</span>
              </div>
