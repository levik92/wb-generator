// Shared logic for product card generation
...
export async function generateProductCards(request: GenerationRequest): Promise<GenerationResult> {
  const { 
    productName, 
    category, 
    description, 
  ...
  if (profileData.tokens_balance < tokensNeeded) {
    throw new Error('Недостаточно токенов для генерации');
  }
  ...
  try {
    ...
    if (error.message !== 'Недостаточно токенов для генерации') {
      try {
