      ...
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-3-pro-image-preview:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            imageConfig: {
              imageSize: imageResolution,  // "1K" or "2K" — uppercase required by Gemini API
              aspectRatio: "3:4"
            }
          }
        }),
...
ОБЯЗАТЕЛЬНО: Верни сгенерированное ИЗОБРАЖЕНИЕ карточки товара.`;
    ...
    let generatedImageBase64 = aiData.candidates?.[0]?.content?.parts?.find((part: any) => part.inlineData)?.inlineData?.data;
