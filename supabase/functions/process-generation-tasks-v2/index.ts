    ...
    while (Date.now() - startTime < maxProcessingTime) {
      ...
      if (!tasks || tasks.length === 0) {
        ...
        if (!pendingTasks || pendingTasks.length === 0) {
          ...
          // История и уведомления теперь создаются автоматически через database trigger
          // при изменении статуса job на 'completed'. Это гарантирует сохранение даже если
          // клиент офлайн или Edge Function завершилась раньше времени.
          console.log(`Job ${job.id} marked as ${finalStatus}, trigger will handle history & notifications`);
