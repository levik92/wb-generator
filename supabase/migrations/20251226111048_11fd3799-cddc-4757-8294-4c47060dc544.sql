-- Delete unused Gemini prompts
DELETE FROM ai_prompts WHERE model_type = 'google' AND prompt_type IN ('clean', 'comparison', 'usage');

-- Update handle_new_user function to give 20 tokens instead of 15
...
BEGIN
    ...
    IF referral_code_from_meta IS NOT NULL AND referral_code_from_meta != '' THEN
        ...
        ELSE
            ...
            VALUES (NEW.id, NEW.email, ref_code, 20);
            
            INSERT INTO public.token_transactions (user_id, amount, transaction_type, description)
            VALUES (NEW.id, 20, 'bonus', 'Стартовые токены при регистрации');
